...
> **Golden Rule:** Content (markdown files) goes in `docs/`. Code (React components, CSS, JS) goes in `src/`. Never mix them.
...
├── build/                                        # ⛔ Auto-generated production output — do not edit
├── .docusaurus/                                  # ⛔ Docusaurus internal cache — do not edit
│
...
The sidebar is configured in `sidebars.js`. Most sections use `autogenerated` to automatically include all docs in a folder — meaning **you don't need to manually list each file**. Docusaurus will pick them up automatically based on the folder path.
...
> **What is "swizzling"?** Docusaurus uses the term *swizzle* to mean overriding a built-in UI component with your own custom version. Files inside `src/theme/` are swizzled components — they replace the Docusaurus defaults. **Do not delete files from `src/theme/`** without understanding what they replace.
...
push.bat
```
This stages **all** changed files, commits with a generic default message, and pushes to `main` in one step.

> ⚠️ **Caution for new hires:** `push.bat` will commit everything that is currently modified — make sure you only have intentional changes staged before running it. If you want a custom commit message or more control, use standard Git commands instead:
> ```bash
> git add .
